...
public class dbKontsultak {
	...
	public static void langileaSartu(Langilea langilea) {
		try {
			...
			String sql_create = "INSERT INTO langilea (`langilea_nan`, `langilea_izena`, `langilea_abizena`, `langilea_telefonoa`, `erabiltzailea`, `pasahitza`, `mota`) "
					+ "VALUES ('" + langilea.nan + "', '" + langilea.izena + "', '" + langilea.abizena + "', '"
					+ langilea.telefonoa + "', '" + langilea.erabiltzailea + "', '" + langilea.pasahitza
					+ "', 'langilea')";
	...
	public static List<Paketea> getPaketeak() {
		...
		try {
			Statement stmt = konexioa.konexioaHasi().createStatement();
			ResultSet rs = stmt.executeQuery(
					"SELECT langilea.langilea_izena, langilea.langilea_abizena, bezeroa.bezeroa_helbidea, paketea.entrega_data, paketea.ID "
	...
	public static void paketeaSartu(Paketea paketea) {
		try {
			Statement stmt = konexioa.konexioaHasi().createStatement();
			String sql_create = "INSERT INTO `paketea` (`ID`, `entrega_data`, `langilea_nan`, `bezeroa_nan`, `zein_entregatu`, `entregatuta`) VALUES (NULL, "
					+ "'" + paketea.entrega_data + "', '" + paketea.langilea_nan + "', '" + paketea.bezeroa_nan
					+ "', '0', '0')";
			stmt.executeUpdate(sql_create);
			JOptionPane.showMessageDialog(null, "datuak ondo sartu dira.");
			konexioa.konexioaItxi();
