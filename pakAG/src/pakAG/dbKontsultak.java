...
public class dbKontsultak {
    ...
    public static List<Historiala> paketeHistoriala() {
        ...
        try {
            Statement stmt = konexioa.konexioaHasi().createStatement();
            ResultSet rs = stmt.executeQuery("SELECT `historiala`.`entrega_data`, `historiala`.`oharra`, `bezeroa`.`bezeroa_izena`, `bezeroa`.`bezeroa_abizena`, `bezeroa`.`bezeroa_helbidea` "
            		+ "FROM `historiala` INNER JOIN `bezeroa` ON`historiala`.`bezeroa_nan` = `bezeroa`.`bezeroa_nan`");
            while (rs.next()) {
                ...
                String oharra = rs.getString("oharra");
                Historiala zeregina = new Historiala(data, izena, helbidea, oharra);
                historialaList.add(zeregina);
            }
            konexioa.konexioaItxi();
        } catch (SQLException e) {
            e.printStackTrace();
